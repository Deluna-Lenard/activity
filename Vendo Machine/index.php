...
<?php
function calculateTotalPrice($basePrice, $sizeAdjustment, $quantity) {
    return ($basePrice + $sizeAdjustment) * $quantity;
}

function displayPurchaseSummary($products, $productPrices, $size, $sizeAdjustments, $quantity) {
    $totalItems = 0;
    $totalPrice = 0;
    ...
    foreach ($products as $product) {
        ...
        $totalPricePerItem = calculateTotalPrice($basePrice, $sizeAdjustment, $quantity);

        $totalItems += $quantity;
        $totalPrice += $totalPricePerItem;

        echo "<ul><li><label>{$quantity} " . ($quantity > 1 ? "pieces" : "piece");
        echo " of {$size} {$product} amounting to ₱" . htmlspecialchars($totalPricePerItem) . "</label></li></ul>";
    }

    echo "<hr><b>Total Items:</b> " . htmlspecialchars($totalItems) . "<br>";
    echo "<b>Total Price:</b> ₱" . htmlspecialchars($totalPrice);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['checkout']) && isset($_POST['product'])) {
        ...
        if ($quantity > 0) {
            displayPurchaseSummary($products, $productPrices, $size, $sizeAdjustments, $quantity);
        } else {
